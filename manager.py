...
class GameManager:
    ...
    def raw_draw_logic(self):
        ...
        # 2. 덱 보충 로직 (카드가 0장일 때 작동)
        if self.card_dummy.isEmpty():
            if self.trash_card.cardList:
                # 버린 카드가 있다면 섞어서 다시 덱으로 만듦
                self.refill_dummy_from_trash()
            else:
                # 버린 카드도 없다면 아예 새로운 세트를 불러옴
                self.generate_emergency_cards()

        # 3. 보충된 덱에서 카드를 뽑음 (위에서 무조건 채워지므로 흐름이 안 끊김)
        ...
        if new_card.card_type == "즉발":
            ...
            if new_card.name == "중간고사":
                ...
                self.draw_card_from_dummy(random.randint(1, 3))
            ...
            # 예시: 깜짝이야! 카드 처리 로직 수정
                ...
                while True:
                    if self.card_dummy.isEmpty() and not self.trash_card.cardList: break
                ...
                for tc in targets:
                    if tc.card_type == "행동": self.add_action(tc)
                ...
                while self.card_on_hand.cardList: self.trash_card.cardList.append(self.card_on_hand.cardList.pop())
    ...
    def draw_card_from_dummy(self, count=1):
        ...
        if self.is_resolving_draws: return
        self.is_resolving_draws = True
        while self.draw_queue_count > 0:
    ...
    def execute_action_card(self, card):
        ...
        if card.name == "게임": self.draw_card_from_dummy(3)
        elif card.name in ["소리 질러", "하루하루"]: self.draw_card_from_dummy(2)
        elif card.name == "밥 먹을 시간이다": self.draw_card_from_dummy(len(self.card_on_hand.cardList) * 2)
        elif card.name == "빛 빛 빛":
            if random.random() < 0.5: self.draw_card_from_dummy(4)
            else:
                for _ in range(3):
                    if self.card_on_hand.cardList: self.trash_card.cardList.append(self.card_on_hand.cardList.pop())
        elif card.name == "준민이는 씨리얼 1짱":
            if self.last_played_card_name == "집 나간 승찬이":
                while len(self.card_on_hand.cardList) > 6: self.trash_card.cardList.append(self.card_on_hand.cardList.pop())
                while len(self.card_on_hand.cardList) < 6: self.raw_draw_logic()
        elif card.name == "집 나간 승찬이": self.draw_card_from_dummy(15)
        ...
        elif card.name in ["어떻게 토큰이 1억개야?", "무명 카드"]: self.draw_card_from_dummy(100)
        elif card.name == "나는야 환경지킴이":
            if self.trash_card.cardList: self.card_on_hand.cardList.append(self.trash_card.cardList.pop())
        ...
        elif card.name == "뽑으세요!": self.draw_card_from_dummy(random.randint(2, 5))
        ...
        elif card.name == "개한테 물렸어!": self.dog_bite_turns = 3
        elif card.name == "변신이다!":
            ...
            while True:
                ...
                self.raw_draw_logic()
                if len(self.card_on_hand.cardList) > old_len and self.card_on_hand.cardList[-1].card_type == "함정": break
            ...
            for c in self.card_on_hand.cardList:
                if any(n in c.name for n in names_2_5): self.trash_card.cardList.append(c)
                else: kept.append(c)
            self.card_on_hand.cardList = kept
    ...
    def check_passive_traps(self):
        ...
        if has_woojin:
            for c in [x for x in self.card_on_hand.cardList if x.name == "설"]:
                self.card_on_hand.cardList.remove(c); self.trash_card.cardList.append(c); self.draw_card_from_dummy(3)
            for c in [x for x in self.card_on_hand.cardList if x.name == "정"]:
                self.card_on_hand.cardList.remove(c); self.trash_card.cardList.append(c); self.raw_draw_logic()
            for c in [x for x in self.card_on_hand.cardList if x.name == "이"]:
                self.card_on_hand.cardList.remove(c); self.trash_card.cardList.append(c)
                for _ in range(2):
                    if self.card_on_hand.cardList: self.trash_card.cardList.append(self.card_on_hand.cardList.pop(random.randrange(len(self.card_on_hand.cardList))))
    ...
    def tick_turn_counters(self):
        if self.turtle_neck_active:
            ...
            if self.turtle_neck_counter <= 0:
                if self.mingi_project_turns > 0: 
                    ...
                    self.trash_card.cardList.extend(self.card_on_hand.cardList)
                    self.card_on_hand.cardList.clear()
                    self.turtle_neck_active = False
        ...
        if self.dog_bite_turns > 0:
            ...
            # 패가 없을 때 에러가 나지 않도록 safe_pop_random 사용
            popped_card = self.safe_pop_random(self.card_on_hand.cardList)
            if popped_card:
                self.trash_card.cardList.append(popped_card)
    ...
    def safe_pop_random(self, target_list):
        """리스트가 비어있지 않을 때만 안전하게 무작위로 pop"""
        if target_list:
            return target_list.pop(random.randrange(len(target_list)))
        return None
    def generate_emergency_cards(self):
        """카드가 모두 고갈되었을 때 새로운 카드 세트를 덱에 꽉 채웁니다."""
        self.add_log("카드가 모두 고갈되어 새로운 카드 세트를 불러옵니다!")
        
        # 게임 시작 시와 동일하게 60장의 카드를 무작위로 생성하여 더미에 넣음
        for _ in range(60): 
            self.card_dummy.push(Card(random.choice(CARD_POOL)))