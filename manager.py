...
class GameManager:
    def __init__(self):
        ...
        self.draw_queue_count = 0        # 뽑아야 할 남은 카드 수
    ...
    def raw_draw_logic(self):
        ...
        # 4. 즉발 카드 로직 (패로 들어가지 않고 즉시 발동 후 버린 카드 더미로 이동)
        if new_card.card_type == "즉발":
            ...
            if new_card.name == "중간고사":
                ...
                self.draw_card_from_dummy(count)
        ...
        # 5. 즉발이 아닌 일반 카드 (패로 들어감)
            ...
            if new_card.name == "게임":
                if any(c.name == "그리다 만" for c in self.card_on_hand.cardList):
                    # 카드가 증발하지 않게 버린 더미로 이동시킵니다.
                    self.trash_card.cardList.extend([c for c in self.card_on_hand.cardList if c.name != "게임"])
                    self.card_on_hand.cardList = [c for c in self.card_on_hand.cardList if c.name == "게임"]

    # 들여쓰기 오류 해결 완료
    def draw_card_from_dummy(self, count=1):
        # 뽑아야 할 카드 장수를 큐에 누적합니다.
        self.draw_queue_count += count
        
        # 만약 이미 드로우 루프가 실행 중이라면, 큐에 장수만 더해주고 즉시 빠져나옵니다. (재귀 스택 방지!)
        if self.is_resolving_draws:
            return
            
        # 드로우 루프가 실행 중이 아니라면 시작합니다.
        self.is_resolving_draws = True
        
        # 예약된 카드가 0장이 될 때까지 안전하게 순차적으로 뽑습니다.
    ...
    def execute_action_card(self, card):
        ...
        if card.name == "게임":
            ...
            self.draw_card_from_dummy(len(self.card_on_hand.cardList) * 2)
            ...
            self.draw_card_from_dummy(random.randint(2, 5))
    ...
    def generate_emergency_cards(self):
        """버린 카드도 없을 때 비상용 카드를 생성합니다."""
        self.add_log("카드가 완전히 고갈되어 새로 생성합니다!")
        for _ in range(30):
            self.card_dummy.push(Card(random.choice(CARD_POOL)))